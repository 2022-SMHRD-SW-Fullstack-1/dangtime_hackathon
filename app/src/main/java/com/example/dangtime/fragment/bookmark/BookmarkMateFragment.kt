...
class BookmarkMateFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_bookmark_mate, container, false)
        val rvBookMate = view.findViewById<RecyclerView>(R.id.rvBookMate)

        getLikePostData()

        adapter = BookmarkAllAdapter(requireContext(), postList, likeMemberList, postImageList)

        rvBookMate.adapter = adapter
        rvBookMate.layoutManager = GridLayoutManager(requireContext(), 1)
        ...
        postRef.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                for (model in snapshot.children) {
                    ...
                    if (item != null && likeList.contains(model.key) && item.category == "mate") {
                        postList.add(item)
